...
docker compose run --rm --entrypoint bash mindstone-agent-pi -lc './scripts/show-isolation.sh; ./scripts/init-runtime.sh; ./scripts/start-gateway.sh >/tmp/gateway.log 2>&1 & pid=$!; sleep 1; node -e '\''for (const path of ["/health", "/status"]) { const r=await fetch(`http://127.0.0.1:19789${path}`); if(!r.ok) process.exit(1); const body=await r.json(); if(!body.ok) process.exit(1); if (path === "/status" && (!body.config?.exists || !Array.isArray(body.agents) || body.agents.length < 1)) process.exit(1); console.log(path); console.log(JSON.stringify(body, null, 2)); }'\''; kill $pid'
