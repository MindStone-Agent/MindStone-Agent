...
docker compose run --rm --entrypoint bash mindstone-agent-pi -lc './scripts/show-isolation.sh; ./scripts/start-gateway.sh >/tmp/gateway.log 2>&1 & pid=$!; sleep 1; node -e '\''const r=await fetch("http://127.0.0.1:19789/health"); if(!r.ok) process.exit(1); const body=await r.json(); if(!body.ok) process.exit(1); console.log(JSON.stringify(body, null, 2));'\''; kill $pid'
