...
grep -q "gateway.endpoint" <<<"$output"
grep -q "gateway.auth" <<<"$output"
grep -q "gateway.http" <<<"$output"
...
printf '%s\n' "$status_output"
grep -q "Gateway: http://127.0.0.1:19789" <<<"$status_output"
grep -q "Gateway auth mode: none" <<<"$status_output"
grep -q "Gateway auth source: not required" <<<"$status_output"
grep -q "Gateway models API enabled: false" <<<"$status_output"
grep -q "Gateway chat completions enabled: false" <<<"$status_output"
grep -q "Gateway responses enabled: false" <<<"$status_output"
