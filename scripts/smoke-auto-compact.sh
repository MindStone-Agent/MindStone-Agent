...
if (compactEvent.metadata?.compaction?.available !== false) process.exit(1);
if (compactEvent.metadata?.compaction?.substrate !== "none") process.exit(1);
if (compactEvent.metadata?.compaction?.reason !== "substrate_compaction_not_available_for_routing_mode") process.exit(1);
