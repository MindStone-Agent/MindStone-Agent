...
export function getMindStoneChannelCatalog(config?: MindStoneConfig): MindStoneChannelCatalog {
  ...
  const entries: MindStoneChannelCatalogEntry[] = [
    ...
    },
    {
      ...
      enabled: channelConfigured(config, "discord"),
      capabilities: { chatTypes: ["direct", "channel"], media: true, nativeCommands: false },
      summary: "Gateway-owned Discord connector (Gateway WebSocket, least-privilege intents) on the shared connector framework.",
      setup: "Configure channels.discord with a token REF (tokenEnv/tokenFile) + allowedGuilds + allowedSenders; the Gateway opens the Discord gateway connection. Live validation steps in docs/operations/LIVE_UAT_RUNBOOK.md.",
      notes: [
        "Access fails closed (empty allowlist = nobody; empty allowedGuilds = DMs only)",
        "Identify intents are least-privilege: GUILD_MESSAGES + DIRECT_MESSAGES + MESSAGE_CONTENT only",
        "Guild replies use message_reference; native thread-channels treated as regular channels (MVP boundary)",
        "Live-Discord validation pending (smoke uses a local stub REST + gateway WS)",
      ],
    },
    {
      id: "slack",
    ...
    },
    {
      ...
      kind: "external_channel",
      status: "available",
      configured: channelConfigured(config, "email"),
      enabled: channelConfigured(config, "email"),
      capabilities: { chatTypes: ["direct", "thread"] },
      summary: "Gateway-owned Gmail connector: scoped mailbox poll in, APPROVAL-GATED replies out (drafts never auto-send), on the shared connector framework.",
      setup: "Configure channels.email with three REFS (tokenEnv=refresh token, clientIdEnv, clientSecretEnv) + allowedSenders/allowedSenderDomains + a scoped Gmail query. Decisions via mindstone approvals. Live validation steps in docs/operations/LIVE_UAT_RUNBOOK.md.",
      notes: [
        "Access fails closed (empty allowlists = nobody; domain trust via allowedSenderDomains)",
        "defaultSendPolicy=approval_required: replies become pending ProposedActions — approve to send, reject to archive (auditable)",
        "Outbound is REPLY-ONLY (recipient/threading reconstructed from the original message; no compose path)",
        "Memory proposals from email are approval-gated (never auto-written)",
        "Live-Gmail validation pending (smoke uses a local stub Gmail API + OAuth endpoint); M365/Graph is a documented second pass",
      ],
    },
    {
      id: "calendar",
