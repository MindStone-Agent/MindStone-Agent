...
export function getMindStoneChannelCatalog(config?: MindStoneConfig): MindStoneChannelCatalog {
  ...
  const entries: MindStoneChannelCatalogEntry[] = [
    ...
    },
    {
      ...
      kind: "gateway_api",
      status: responsesEnabled ? "not_validated" : "planned",
      configured: true,
      enabled: responsesEnabled,
      capabilities: { chatTypes: ["direct"] },
      summary: "Gateway /v1/responses-compatible surface planned from MindStone lineage.",
      setup: "Enable gateway.http.responses only after implementation/validation catches up.",
      notes: ["Not validated for MVP yet"],
    },
    {
      id: "openwebui",
      label: "OpenWebUI",
    ...
    },
    {
      ...
      enabled: false,
      capabilities: { chatTypes: ["direct", "group"], media: true, nativeCommands: true },
      summary: "Planned Gateway-owned Telegram Bot API channel plugin.",
      setup: "Future setup should collect token source, bot identity, allowlists, and session policy without exposing secrets.",
      notes: ["Not implemented/validated in MindStone-Agent yet", "Best first external channel candidate"],
    },
    {
      id: "discord",
      label: "Discord",
    ...
    },
    {
      ...
      enabled: false,
      capabilities: { chatTypes: ["direct", "channel", "thread"], reactions: true, media: true, nativeCommands: true },
      summary: "Planned Gateway-owned Slack channel plugin.",
      setup: "Future setup should collect app/bot token sources, Socket Mode/webhook choice, workspace/channel allowlists, and scopes.",
      notes: ["Not implemented/validated in MindStone-Agent yet"],
    },
    {
      id: "signal",
