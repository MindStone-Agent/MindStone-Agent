...
- [x] Add conservative Pi adapter `session_shutdown` lifecycle marker hook that appends a sanitized `pi_adapter_session_shutdown` event to the configured MindStone transcript; this is not full raw Pi transcript archive parity yet.
- [x] Add first-pass Pi adapter `before_agent_start` prompt-context injection for configured `IDENTITY.md` and `USER.md`, verified without live model calls.
