...
- [x] Add `mindstone memory maintain` and `mindstone memory backfill --maintain` for memory substrate maintenance: stale-source cleanup, opt-in exact-text dedupe, orphan-source cleanup, SQLite optimize/reindex/VACUUM, WAL checkpointing, and bloat diagnostics.
