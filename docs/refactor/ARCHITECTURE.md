...
MindStone-Agent's user-facing terminal path follows the current MindStone/OpenClaw pattern: a MindStone-owned TUI app built from Pi TUI primitives, backed by Gateway/Core routing and Pi session execution. The first `mindstone tui` shell uses vendored Pi TUI components (`TUI`, `ProcessTerminal`, `Editor`, `Markdown`, `Loader`, `Container`) with MindStone styling, loads recent transcript history on startup, starts a mutable assistant message while a turn is running, renders returned turn events as compact event lines, exposes `/status`, `/config`, `/gateway`, `/pi`, `/transcript`, `/memory`, `/context`, `/handoff`, `/identity`, `/events`, `/runs`, and `/doctor` panels with runtime/config/Gateway/Pi-session/transcript/recall/context/handoff/identity/event/run/doctor visibility, includes `/sessions`, `/agents`, and `/models` panels, supports runtime-only `/session <key>`, `/agent <id>`, and `/model <id>` switching without config mutation, renders `AgentRunner.stream(...)` lifecycle/diagnostic events during turns with richer substrate/tool labels, and applies completed-response `text_delta` replay events to the mutable assistant message. `mindstone chat` is also an MVP native terminal surface over the same canonical session/routing path; when interactive placeholder routing would otherwise dead-end, it now starts routing/model setup in-place before opening chat. Live token/tool event streaming and richer selector/settings overlays remain pending.
