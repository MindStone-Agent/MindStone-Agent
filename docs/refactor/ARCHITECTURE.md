...
MindStone-Agent's user-facing terminal path follows the current MindStone/OpenClaw pattern: a MindStone-owned TUI app built from Pi TUI primitives, backed by Gateway/Core routing and Pi session execution. The first `mindstone tui` shell uses vendored Pi TUI components (`TUI`, `ProcessTerminal`, `Editor`, `Markdown`, `Loader`, `Container`) with MindStone styling, loads recent transcript history on startup, starts a mutable assistant message while a turn is running, renders returned turn events as compact event lines, exposes a first `/status` panel with agent/session/route/model/provider/config/transcript/runtime details, includes non-mutating `/sessions`, `/agents`, and `/models` panels, renders `AgentRunner.stream(...)` lifecycle/diagnostic events during turns, and applies completed-response `text_delta` replay events to the mutable assistant message. Live token/tool event streaming and interactive selectors/overlays remain pending. The raw `mindstone chat` command remains a plumbing/test harness.
