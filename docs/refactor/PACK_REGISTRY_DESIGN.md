...
- **A knowledgebase pack dir may contain ONLY `kb.json` and files under `sources/`** (whitelist, casing-robust — adversarial QA). A shipped `index.json` (in ANY casing — the KB loader resolves it case-insensitively on macOS/Windows) or any other stray file is refused: the index is *generated content* that would reach recall unreviewed, bypassing the `sources/*.md` surface enumeration. It MUST be built post-install (`mindstone kb ingest`) from the reviewed, digest-covered sources.
- **A packed `kb.json` is third-party-authored, so it is validated as untrusted config** (the KB trust model's "operator-authored" assumption holds only for operator-written kb.json, not a pack's). Its `name`/`description` are injected verbatim into recall (`Knowledgebase "<name>" …`), so they must be single-line labels ≤200 chars with no double-quote or control characters (a hostile name would break the quote and inject instructions). It may declare **no `externalSources`** — those steer `kb ingest` to read arbitrary host folders / fetch URLs into recall, content never in the pack and never reviewed; the operator adds external sources post-install (that path is operator-authored and trusted). With the whitelist + this, everything a packed KB injects is either a reviewed `sources/*.md` or a bounded-safe label.
...
- **`promptSurfaces` derivation is a fixed, versioned rule** (`promptSurfacesRule`), implemented identically by the pack build tool and the installer so build-time and install-time derivations cannot drift into false integrity failures. **Rule 1 (hardened after adversarial QA):** the derived set is **every archive-relative path whose name ends in `.md`, matched case-INsensitively**, plus the declared `identitySeed` file even if it is not markdown — as a byte-wise-sorted list of POSIX-style relative paths, compared by **exact list equality** (a path-set comparison; no content canonicalization is involved). This is deliberately *loader-agnostic and exhaustive* rather than a hand-listed subset (`PERSONA.md`/`safety.md`/`SKILL.md`/memory seeds): the runtime loaders that inject markdown into the model context do not share one naming convention — memory-seed recall matches `.md` **case-insensitively** (so `EVIL.MD` loads), and knowledgebase markdown sources are ingested into context too — so any hand-listed subset risks missing a file a loader will inject. Enumerating every `.md` closes that gap; over-listing a non-loaded `.md` is harmless (exact-match still holds), while missing a loaded one is the whole integrity hole. A manifest whose `promptSurfaces` differs from the derived set is an integrity FAILURE, not a warning — an undeclared prompt surface is exactly what an attacker would want. An installer that does not know the manifest's `promptSurfacesRule` version refuses to install (fail-closed, same posture as unknown `schemaVersion`). Prompt text embedded in JSON artifacts (workflow step prompts, `skill.json` / `kb.json` fields) is covered by the per-file digests and by review of the artifact as a whole; `promptSurfaces` enumerates the *markdown prompt documents*.
