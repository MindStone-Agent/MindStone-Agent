...
`source.senderId`, not scope). When the mailbox is the employee's own user account, `userId` is left unset and the entry
carries `tenantId` and `agentId` only, because the scope filter (`app-engine/types.ts`,
...
for a delegated or shared mailbox owned by a person. The intent is that mail is the employee's
memory, not one person's. An `engagement`
...
- No enterprise-support claims until the L1 live leg defined in `TEAMS_CONNECTOR_DESIGN.md` §7
  ...
  claim is exactly what the leg validated.
