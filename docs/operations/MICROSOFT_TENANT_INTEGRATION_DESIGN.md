...
`tenantId` and `agentId` only, because the scope filter (`app-engine/types.ts`,
...
- No enterprise-support claims until the L1 live leg defined in `TEAMS_CONNECTOR_DESIGN.md` §7
  ...
  against real Graph, Storage, and Search audiences.
